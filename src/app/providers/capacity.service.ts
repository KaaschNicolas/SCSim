...
export class CapacityService {
    ...
    public async capacityOrdersInWork(capacityContainer: WorkingStationCapacityContainerDto) {
        ...
        (
            ...
            let ppItem = await this.itemRepository.findOne({
                where: { itemNumber: orderInWork.itemId },
            });
            let ppWorkingStation = await this.workingStationRepository.findOne({
                where: { number: orderInWork.workingStationId },
            });
            let productionProcess = await this.productionProcessRepository.find({
                where: {
                    item: ppItem,
            ...
            (
                await this.entityManager.getTreeRepository(ProductionProcess).findDescendants(productionProcess[0], { relations: ['workingStation', 'item'] })
        ...
        capacityContainer.workingStationCapacities.forEach(async (capacity) => {
